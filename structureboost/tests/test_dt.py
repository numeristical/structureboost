...
def test_get_mask_int_c():
    len_to_use = 10
    asd = np.arange(len_to_use).astype(np.int64)
    inq_set = np.array([3, 5, 6]).astype(np.int64)
    mv = np.zeros(len_to_use).astype(np.int64)
...
def test_get_bin_sums_c():
    ...
    brv = np.searchsorted(zxc,asd).astype(np.int64)
