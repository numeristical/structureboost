...
def get_basic_config_series(feature_vec, default_config):
    ...
    if feature_vec.dtype == 'O':
        ...
        config_dict['graph'] = graphs.complete_graph(pd.unique(feature_vec))
        ...
        if categorical_method == 'span_tree':
            config_dict['num_span_trees'] = default_config['default_num_span_trees']
    else:
        config_dict['feature_type'] = 'numerical'
        config_dict['max_splits_to_search'] = default_config['default_numerical_max_splits_to_search']
...
def default_config_dict():
    ...
    config_dict['default_categorical_max_splits_to_search'] = 25
    config_dict['default_numerical_max_splits_to_search'] = 50
    return config_dict
...
def ice_plot(model, data_row, column, range_pts, add_nan=True):
    ...
    if add_nan:
        plt.hlines(pred_vals[-1],xmin=range_pts[0], xmax=range_pts[int(np.floor(len(range_pts)/4+1))],
